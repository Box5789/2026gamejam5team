...
## 2026-06-28 Kitchen EditMode Tests Player Build Exclusion

- Fixed the Player build compiler errors from Kitchen tests being compiled into the runtime assembly.
  - Root cause: `Assets/Tests/EditMode/Kitchen/*.cs` lived under an `EditMode` folder but had no Editor-only assembly boundary, so Unity included them in `Assembly-CSharp` during Player build.
  - That made Player compilation see Editor-only APIs such as `UnityEditor.SceneManagement.EditorSceneManager` and `UnityEditor.SerializedObject`.
- Moved all direct Kitchen EditMode test `.cs` files and matching `.cs.meta` files into `Assets/Tests/EditMode/Kitchen/Editor/`.
  - Added `Assets/Tests/EditMode/Kitchen/Editor.meta`.
  - Kept existing `.cs.meta` files with the moved tests so test asset GUIDs are preserved.
  - Did not change the existing Spreading asmdef tests or Kitchen runtime asmdef structure.
- Validation:
  - Local generated `Assembly-CSharp.csproj` was refreshed for validation so direct `Assets\Tests\EditMode\Kitchen\*.cs` references are gone and Kitchen tests appear only under `Assembly-CSharp-Editor.csproj`.
  ...
  - Unity batchmode EditMode tests were attempted but Unity reported `HandleProjectAlreadyOpenInAnotherInstance`, so no test result XML was produced.
- Notes:
  - `.csproj` and `.sln` files are ignored/generated by Unity; the durable project change is the test file move into the `Editor` folder.
  - Existing unrelated Unity/user changes in `order.unity`, ProjectSettings, URP assets, and PerformanceTestRunInfo/Settings were left untouched.
